...
async def poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.effective_message.reply_text(
            "사용법: /p [제목] [경기1] [경기2]\n"
            "예시: /p 야구승패예측 요미우리vs한신 지바롯데vs세이부"
        )
        return
    title = context.args[0]
    matches = context.args[1:]
    now = datetime.now()
    poll_id = str(now.strftime('%H%M%S'))
    polls[poll_id] = {
        'title': title,
        'matches': {},
        'voters': {},
    ...
    for i, match in enumerate(matches, 1):
        ...
        if len(teams) == 2:
            ...
            row = [
                InlineKeyboardButton(f"{teams[0]}", callback_data=f"{poll_id}:{match}:team1"),
                InlineKeyboardButton("무승부", callback_data=f"{poll_id}:{match}:draw"),
                InlineKeyboardButton(f"{teams[1]}", callback_data=f"{poll_id}:{match}:team2"),
            ]
            keyboard.append(row)
    text += f"\n━━━━━━━━━━━━━━\n"
    text += f"✅ 각 경기당 1회만 투표 가능"
    keyboard.append([InlineKeyboardButton("📊 실시간 현황", callback_data=f"{poll_id}:all:result")])
...
async def endpoll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for match, data in poll_data['matches'].items():
        ...
        draw = len(data['draw']['voters'])
        total = t1 + t2 + draw
        result_text += f"\n⚽ {teams[0]} vs {teams[1]}\n"
        if total > 0:
            t1_pct = int(t1 / total * 10)
            draw_pct = int(draw / total * 10)
            t2_pct = int(t2 / total * 10)
            result_text += f"{teams[0]}: {'🟦' * t1_pct}{'⬜' * (10-t1_pct)} {t1}표\n"
            result_text += f"무승부: {'🟨' * draw_pct}{'⬜' * (10-draw_pct)} {draw}표\n"
...
async def voters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for match, data in poll_data['matches'].items():
        ...
        result_text += f"무승부: {', '.join(data['draw']['voters']) or '없음'}\n"
...
async def vote_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer(f"✅ 투표 완료!", show_alert=True)
